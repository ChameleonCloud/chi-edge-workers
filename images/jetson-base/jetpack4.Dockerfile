...
# nvidia-container-csv-cuda depends on cuda-toolkit-10-2 (~3GB), but the
# package itself is just a single CSV file that tells the container runtime
# which host paths to mount. Install it without pulling the dependency.
RUN apt-get update \
